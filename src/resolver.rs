...
impl DependencyResolver {
    ...
    pub fn resolve(&mut self, manifest: &Manifest) -> anyhow::Result<Vec<ResolvedPackage>> {
        ...
        let mut to_resolve: VecDeque<(String, usize, Option<String>, Option<String>)> = VecDeque::new();

        // Add root dependencies
        for (name, version_req_str) in &manifest.dependencies {
            let version_req = self.parse_version_req(version_req_str)?;
            requirements.entry(name.clone()).or_insert_with(Vec::new).push(version_req.clone());
        ...
        while let Some((name, depth, _source, source_spec)) = to_resolve.pop_front() {
            ...
            let version_reqs = requirements.get(&name)
                .ok_or_else(|| anyhow::anyhow!("No version requirements found for package: {}", name))?;
                ...
                if let Some(existing) = resolved.get(dep_name) {
                    ...
                    if !dep_version_req.matches(&existing.package.version) {
                        return Err(anyhow::anyhow!(
                            ...
                            name, dep_name, dep_version_req, dep_name, existing.package.version
                        ));
                    }
                } else {
    ...
    fn find_compatible_version(&mut self, name: &str, version_reqs: &[VersionReq]) -> anyhow::Result<Package> {
    ...
    fn find_package_from_source(&mut self, name: &str, source_spec: &str, version_reqs: &[VersionReq]) -> anyhow::Result<Package> {
        ...
        if source_spec.starts_with("file://") {
            ...
            return Err(anyhow::anyhow!(
                ...
                name, path_str, package.version, version_reqs
            ));
        } else if source_spec.starts_with("git://") || source_spec.starts_with("git+") {
            // Git repository
            let url = source_spec.strip_prefix("git://")
            ...
            return Err(anyhow::anyhow!(
                ...
                name, url, package.version, version_reqs
            ));
        }
    ...
    fn parse_version_req(&self, version_str: &str) -> anyhow::Result<VersionReq> {
        ...
        let version_str = if version_str.starts_with("file://") 
